...
public enum Event {
    D_DAY("크리스마스 디데이 할인", IntStream.range(Constants.MIN_VISIT_DATE, Constants.D_DAY_LAST_DAY).boxed().collect(Collectors.toList()), 100),
    ...
    PRESENTATION("증정 이벤트", IntStream.range(Constants.MIN_VISIT_DATE, Constants.MAX_VISIT_DATE).boxed().collect(Collectors.toList()), 25000);
